...
class StateEstimator:
	...
	def __init__(self, robot, T):
		...
		self.force_threshold = 2.7 # Foot contact force threshold
		self.contact_hysteresis_samples = 4 #8 for LORD
	...
	def calibrate(self):
		...
		if self.cal_i is None:
			...
			q_temp = quaternion_multiply_JPL(q_mean, q_rotate)
	...
	def predict_state(self):
		...
		for j in range(0,self.robot.active_legs):
			...
			if th > self.contact_hysteresis_samples:
			...
			if th > self.contact_hysteresis_samples: # LORD IMU at 500 Hz
				cov = T * C.T.dot(self.Qp).dot(C)
