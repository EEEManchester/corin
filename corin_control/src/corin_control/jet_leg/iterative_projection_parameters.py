...
#        print 'number of elements: ', num_of_elements
        local_optimization_started2 = False
        for j in range(0, num_of_elements):
            if str(received_data.name[j]) == str("optimization_started"):
                local_optimization_started2 = bool(received_data.data[j])

        if local_optimization_started2 is True:
...
# the in    puts are all in the WF this way we can compute generic regions for generic contact sets and generic com position
...
#                    self.normals[3,2] = received_data.data[j]
    ...
    def getFutureStanceFeet(self, received_data):
        ...
        num_of_elements = np.size(received_data.data)
        local_optimization_started0 = False
        for j in range(0, num_of_elements):
            if str(received_data.name[j]) == str("optimization_started"):
                local_optimization_started0 = bool(received_data.data[j])

        if local_optimization_started0 is True:
            print 'Optimization started flag is now true. Reading new data...............'
            for j in range(0,num_of_elements):
    ...
    def getCurrentStanceFeet(self, received_data):
        ...
        for j in range(0, num_of_elements):
            if str(received_data.name[j]) == str("optimization_started"):
                local_optimization_started1 = bool(received_data.data[j])

        if local_optimization_started1 is True:
            print 'Optimization started flag is now true. Reading new data...............'
