...
class RecursiveProjectionProblem(object):
  ...
  def next_edge(self, plot=False, plot_direction=False,
    ...
    if plot or record:
      ...
      if plot:
        self.show()
      if record:
        filename = "fig/stability_{0:04d}".format(number)
  ...
  def plot_polyhedrons(self):
    self.plot_polyhedron(self.inner, 'red', 0.6)
    self.plot_polyhedron(self.outer, 'blue', 0.3)
    # print self.outer
    # print self.inner

  def plot_polyhedron(self, poly, c, a):
    coords, tri = self.triangulate_polyhedron(poly) # backend function
    if len(coords) == 3:
