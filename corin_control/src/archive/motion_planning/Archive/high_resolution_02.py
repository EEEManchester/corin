...
## Medium resolution grid cells - 0.06m^2
...
class grid_planner:
	def __init__(self,size):
		self.cell_size 	= 0.06			# size of cell, (m) - TARGET: 0.03
	...
	def node_cost(self):
		...
		box1 = [(4,11),(8,18)] 		# cell_size = 0.06m
		box2 = [(16,11),(18,22)]
	...
	def check_area(self, p, ctype):
		...
		if (ctype == 'body'):
			...
			bx = (self.rblg_gd[0]-1)/2 # int(math.ceil(((self.rblg_gd[0]))/2.)) 		# longitudinal distance from robot centre
			by = (self.rblg_gd[1]-1)/2 # int(math.ceil(((self.rblg_gd[1]))/2.))		# lateral distance from robot centre
			# print 'bx ', bx, by, self.rblg_gd
			## cycle all six legs
			for i in range(0,6):
				# set foot and transform (x,y) (m)
				pbnom = ( int(np.round(p_leg[i][1]/float(self.cell_size))), int(np.round(p_leg[i][0]/float(self.cell_size))) )
				pwnom = tuple(map(lambda x,y: x+y, pbnom,p))	# leg nominal stance in world frame
				fcost = 0 										# total cost per foot
				# print i, pbnom

				# pwnom = tuple(map(lambda x,y: int(math.ceil(x/self.cell_size))+y, pold,p))	# old pbnom
				# pold = (p_leg[i][1],p_leg[i][0]) 	# old pbnom
				# if (p==(12, 4)):
				# 	print '=============='
				...
				if (p == (12,15)):
					print i, math.floor(fcost/((2.*bx+1)*(2.*by+1)))

				# calculate average of area and add to total
				scost = scost + math.floor(fcost/((2.*bx+1)*(2.*by+1)))
		...
		for e in self.G.nodes():
			...
			if (e[0]<=(ax-1) or e[0]>=(self.mp_sz[0]-ax) or e[1]<=(ay-1) or e[1]>=(self.mp_sz[1]-ay)): 	
				...
				if (body_cost < 1):
					...
					# pwnom = self.G.nodes[(e[0]+ax,e[1]+ay)]['cost'] # top right
					...
					# self.Gbody.nodes[e]['cost'] = int(math.ceil(float(pwnom + p2 + p3 + p4)/4)) 	# average of all foothold area

					# compare
					# if (int(math.ceil(float(pwnom + p2 + p3 + p4)/4)) > footholds_cost):
					# 	print int(math.ceil(float(pwnom + p2 + p3 + p4)/4)), footholds_cost

					# set node cost based on foothold average cost					
					self.Gbody.nodes[e]['cost'] = footholds_cost
					# print self.Gbody.nodes[(12,10)]['cost'] 	# cost = 2
					if (e == (12,15)):
						print self.Gbody.nodes[e]['cost']
	...
	def graph_representation(self):
		...
		nsize  = 200	# size of nodes
		labels = False 	# label on nodes

		ax = nx.draw_networkx(self.Gbody,pos,with_labels=labels,node_size=100,node_color='0.8',width=1.5,node_shape="s");
		# Body path
		nx.draw_networkx(self.Gfree,p_body,with_labels=labels,node_size=nsize,node_color='y',edge_color='r',width=0.0);
		# # Obstacles
		nx.draw_networkx(self.G_obs,p_obs,with_labels=labels,node_size=nsize,node_color='r',width=0.0);
		# # Feasible path - single point
		nx.draw_networkx(self.Gpath,p_fin,with_labels=labels,node_size=nsize-150,node_color='g',edge_color='b',width=3.0);
...
pp.find_path((12, 5),(12, 28))			# define start and end points
