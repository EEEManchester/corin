...
class grid_planner:
	...
	def check_motion(self, p):
		...
		## variables ##
		valid_flag = False
		node_walk  = [] 	# list for walking motion
		node_wall  = [] 	# list for walling motion
		node_chim  = [] 	# list for chimney motion

		## Walking ##
		#  condition: valid foothold exists within foothold area 
		footholds_cost, valid_flag = self.check_walking_leg_area(p)
		# ILLUSTRATION: stack into graph
		if (valid_flag):self.GM_walk.add_node(p)

		## Walling ##
		if (not valid_flag):
			# condition: 1) wall within X.X m from base point, 2) ground is present
			footholds_cost, valid_flag = self.check_walling_leg_area(p)
			if (valid_flag):self.GM_wall.add_node(p)

		## Chimney ##
		if (not valid_flag):
			# print 'try chimney'
			footholds_cost, valid_flag = self.check_chimney_leg_area(p)
			# ILLUSTRATION: stack into graph
			if (valid_flag):self.GM_chim.add_node(p)
		## append to graph
	...
	def primitive_search_path(self):
		...
		for e in self.G.nodes():
			...
			if (e[0]<=(ay-1) or e[0]>=(self.mp_sz[0]-ay) or e[1]<=(ax-1) or e[1]>=(self.mp_sz[1]-ax)): 	
				...
				# compute for nodes that are obstacle free
				if (valid):
					motion_cost, valid_flag = self.check_motion(e)
	...
	def graph_representation(self):
		...
		# Motion primitives
		nx.draw_networkx(self.GM_walk,m_walk,with_labels=labels,node_size=8,node_color='tab:orange',width=0.0,node_shape="d");
		nx.draw_networkx(self.GM_wall,m_wall,with_labels=labels,node_size=8,node_color='tab:cyan',width=0.0,node_shape="d");
		nx.draw_networkx(self.GM_chim,m_chim,with_labels=labels,node_size=8,node_color='red',width=0.0,node_shape="d"); 	# dark grey
...
pp.find_path((30, 10),(30, 56))			# define start and end points
