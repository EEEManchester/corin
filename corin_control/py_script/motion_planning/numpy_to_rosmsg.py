...
def array_to_path(path_arr, stamp=None, frame_id=None):
	...
	for i in range (0,len(path_arr.X.t)):
		...
		pose.pose.position.x = path_arr.X.xp[i][0]
		pose.pose.position.y = path_arr.X.xp[i][1]
		pose.pose.position.z = path_arr.X.xp[i][2]
		
		quat = tf.transformations.quaternion_from_euler(path_arr.W.xp[i][0], 
														path_arr.W.xp[i][1], 
														path_arr.W.xp[i][2]+np.pi/2) 
