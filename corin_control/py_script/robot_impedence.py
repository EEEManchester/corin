...
if __name__ == "__main__":
	...
	force_desired = 7 # Desired foot force in support phase
	...
	positions = {}
	positions[0] = np.array([ STANCE_WIDTH*np.cos(TETA_F*np.pi/180), STANCE_WIDTH*np.sin(TETA_F*np.pi/180), -BODY_HEIGHT ])
	positions[1] = np.array([ STANCE_WIDTH, 0, -BODY_HEIGHT])
	positions[2] = np.array([ STANCE_WIDTH*np.cos(TETA_R*np.pi/180), STANCE_WIDTH*np.sin(TETA_R*np.pi/180), -BODY_HEIGHT ])
	positions[3] = np.array([ STANCE_WIDTH*np.cos(TETA_F*np.pi/180), STANCE_WIDTH*np.sin(-TETA_F*np.pi/180), -BODY_HEIGHT ])
	positions[4] = np.array([ STANCE_WIDTH, 0, -BODY_HEIGHT])
	positions[5] = np.array([ STANCE_WIDTH*np.cos(TETA_R*np.pi/180), STANCE_WIDTH*np.sin(-TETA_R*np.pi/180), -BODY_HEIGHT ])
	...
	while not rospy.is_shutdown():
		i+=1
		manager.Robot.update_state()
		for j in range(6):
			# force = np.linalg.norm(self.robot.Leg[j].F6c.tibia_X_foot[:3])
			force = manager.Robot.Leg[j].F6c.tibia_X_foot[2] - force_desired# z-axis
		...
		qd = manager.Robot.task_X_joint()
		manager.publish_topics(qd)
