...
class Particle():
    ...
    Q_lims = [Qf_lim, Qbf_lim, Qw_lim, Qbw_lim, Qp_lim, Rs_lim, Th_lim]
...
if __name__ == "__main__":
    ...
    for j in range(particles):
        ...
        # new_value = func(swarm[j].position)
        # amended = position_to_cov(swarm[j].position)
        # new_value = state.evaluate(amended)
        # swarm[j].evaluate(new_value)

    # Initiliase swarm best value
    # swarm_best_value = swarm[0].best_value
    # swarm_best_position = swarm[0].best_position.copy()
    # for j in range(particles):
    #     if swarm[j].best_value < swarm_best_value:
    #         swarm_best_position = swarm[j].best_position
    #         swarm_best_value = swarm[j].best_value


    print "initial: ", swarm_best_position, swarm_best_value
    ...
    plt.figure()
    plt.title(str(state.exp_no) + ": Best global position")
    ...
    plt.figure()
    plt.title(str(state.exp_no) + ": Best global value")
