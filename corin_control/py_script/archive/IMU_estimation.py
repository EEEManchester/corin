...
class CorinState:
	def __init__(self):
		...
		self.pub_r = rospy.Publisher('imu_myAHRS_r', Vector3, queue_size=1)
	...
	def Imu_callback(self, msg): 		# robot joint state callback
		...
		print "w stddev:", np.std(self.w_vec, axis=0), "g stddev:", np.std(self.g_vec, axis=0)
		...
		self.pub_r.publish(Vector3(*self.x.tolist()))
