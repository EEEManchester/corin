...
class CorinManager:
	...
	def save_to_yaml(self, motion_plan):
		...
		data = dict(map=mapname,
					...
					start = dict(x = self.PathPlan.start[0]*self.GridMap.resolution,
								 y = self.PathPlan.start[1]*self.GridMap.resolution,
								 z = self.PathPlan.base_map.nodes[self.PathPlan.start]['pose'][0]),
					goal = dict(x = self.PathPlan.goal[0]*self.GridMap.resolution,
								y = self.PathPlan.goal[1]*self.GridMap.resolution),
					path_x_t = xt,
					path_x_xp = xp,
					path_x_xv = xv,
					path_x_xa = xa,
