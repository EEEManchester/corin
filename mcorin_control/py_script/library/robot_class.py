...
class LegClass:
	...
	# def setNominalStance(self, Body, legPhase=2, gait=[]):
	# 	# nom_stance NEEDS TO BE UPDATABLE
	# 	tran_init = param_gait.get_transition()
	# 	tran_pose = param_gait.get_transition_stance()

	# 	if (tran_init == 0):
	# 		stance_pose = np.vstack( (LEG_STANCE[self.number][0],LEG_STANCE[self.number][1], LEG_STANCE[self.number][2],np.matrix('1') ) )	
	# 	else:
	# 		if (self.number <= 3):
	# 			if(legPhase == 1 and self.tran_cycle == 0):
	# 				self.tran_cycle += 1
	# 				stance_pose = np.vstack( (tran_pose[0],tran_pose[1], tran_pose[2],np.matrix('1') ) )
	# 			elif(self.tran_cycle):
	# 				self.tran_cycle += 1
	# 				stance_pose = np.vstack( (tran_pose[0],tran_pose[1], tran_pose[2],np.matrix('1') ) )
	# 			else:
	# 				stance_pose = np.vstack( (LEG_STANCE[self.number][0],LEG_STANCE[self.number][1], LEG_STANCE[self.number][2],np.matrix('1') ) )
	...
	# 	self.nom_stance = Body.H_wb*self.H_bl*self.R_bl*stance_pose

	# 	heading = param_gait.get_heading() 
	# 	direction = param_gait.get_direction()
		
	# 	x_i = direction*(0.5)*(heading[0]*np.cos(float(heading[2])) - heading[1]*np.sin(float(heading[2])))
	# 	y_i = direction*(0.5)*(heading[1]*np.sin(heading[2]) + heading[1]*np.cos(heading[2]))
	# 	#print "mod dir: ", x_i, " ", y_i

	# 	if (tran_init == 0):
	# 		self.AEP = self.nom_stance + np.matrix([ [x_i] ,[y_i],[0],[0] ])
	# 		self.PEP = self.nom_stance + np.matrix([ [-x_i],[-y_i],[0],[0] ])
	# 	else:
	# 		if (self.number <= 3):
	# 			if(legPhase == 1 and self.tran_cycle == 1):
	# 				a = gait.item(0).denominator - gait.item(2,self.number-1)
	# 				b = float(a)/gait.item(0).numerator

	# 				self.AEP = self.nom_stance + np.matrix([ [x_i],[y_i*b],[0],[0] ])
	# 				self.PEP = self.nom_stance + np.matrix([ [-x_i],[0],[0],[0] ])
	# 			else:
	# 				self.AEP = self.nom_stance + np.matrix([ [0],[0],[x_i],[0] ])
	# 				self.PEP = self.nom_stance + np.matrix([ [0],[0],[-x_i],[0] ])

				# print 'Leg ', self.number, ' Nom: ', self.nom_stance.transpose()
		#print 'Leg ', self.number, ' AEP: ', self.AEP.transpose()
		#print 'Leg ', self.number, ' PEP: ', self.PEP.transpose()
