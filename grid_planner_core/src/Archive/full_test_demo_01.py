...
class grid_planner:
	...
	def map_cell_cost(self):
		...
		hole_01 = [(2,25),(22,40)] 	# [(40,80),(60,110)] 	#
		...
		if (self.G.nodes[p]['cost'] != 1):
			...
			for y in range(0,by+1): 				# cycle lateral - inflated if body_area[1] is even
				for x in range(0,body_area[0]):		# cycle longitudinal
					...
					# if (p==(27,70)):
					# 	print (p[0]-y,p[1]-bx+x), EL_edge, (p[0]+y,p[1]-bx+x), ER_edge
				...
				if (i_edge==0):
					...
					for x in range(0,bx_min*2+1):
						try:
							...
							if (p==(20,60)):
								print p,(p[0]-by_min-d,p[1]-bx_min+x),(p[0]+by_min+d,p[1]-bx_min+x),(p[0]-by_min-d,p[1]+bx_min-x),(p[0]+by_min+d,p[1]+bx_min-x)
						except:
							pass
	...
	def check_motion(self, p):
		...
		if (self.advance_capable):
			...
			## Walling ##
			if (not valid_flag and self.check_body_area(p,(8,3))):
	...
	def graph_representation(self):
		...
		nx.draw_networkx(self.GM_wall,m_wall,with_labels=labels,node_size=10,node_color='tab:cyan',width=0.0,node_shape="d");
		nx.draw_networkx(self.GM_chim,m_chim,with_labels=labels,node_size=10,node_color='red',width=0.0,node_shape="d"); 	# dark grey
...
pp = grid_planner((1.27,5.48)) 			# map size, [x,y] (m,m) - (1.8,4.2)
pp.find_path((15, 10),(10,150))			# define start and end points (20,125) 	(20,60)
